...
export function DedupReview({ dataset, user }: { dataset: string; user: { user_id: number; username: string } }) {
  const [pair, setPair] = useState<Pair | null>(null);

  const load = useCallback(async () => { setPair(await dedupNext(dataset, user.user_id)); }, [dataset, user.user_id]);
  ...
  return (
    <div className="dedup" tabIndex={0} onKeyDown={(e) => { if (e.key === "d") void resolve("delete"); if (e.key === "k") void resolve("keep"); }}>
      ...
      <div className="dedup-imgs">
        <figure><img src={imageUrl(dataset, pair.keeper!)} alt="keeper" /><figcaption>keeper {pair.keeper}</figcaption></figure>
