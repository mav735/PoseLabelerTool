...
def _user_image(session):
    u = User(username="u1")
    session.add(u)
    session.add(Image(stem="100", in_bad_labels=True))
    session.commit()
    return u
...
def test_heartbeat_extends_expiry(db_session):
    ...
    acquire(db_session, "bad", u.id, T0, 180)
...
def test_release_active_frees_for_reacquire(db_session):
    u = _user_image(db_session)
    assert acquire(db_session, "bad", u.id, T0, 180) == "100"
    assert acquire(db_session, "bad", u.id, T0, 180) is None
    assert release_active(db_session, u.id, "100", "bad", T0) is True
    assert acquire(db_session, "bad", u.id, T0, 180) == "100"
...
def test_sweep_expired_releases_stale(db_session):
    ...
    assert acquire(db_session, "bad", u.id, future, 180) == "100"
